...
export default function ClinicLocator() {
  ...
  const searchClinics = async (e) => {
    ...
    try {
      ...
      if (locationData.length > 0) {
        ...
        // Note: Nominatim isn't perfect for "category" search in a radius,
        // so we search for "dentist" or "dental clinic" specific to the bounds/area or query
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          // Reverse geocode to get city name for search (optional) OR just search nearby
          // Searching nearby via coordinate string
          try {
            const clinicRes = await fetch(
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-100px)] rounded-xl overflow-hidden shadow-2xl bg-white">
      {/* Top Bar */}
