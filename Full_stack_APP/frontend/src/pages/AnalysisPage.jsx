...
export default function AnalysisPage({ setModels }) {
  // State management
  const [models, setLocalModels] = useState(null);
  ...
  const fetchModels = async () => {
    try {
      ...
      setLocalModels(data);
      setModels(data); // Pass to parent App
    } catch (err) {
  ...
  return (
    <>
      ...
      {result && <ResultsDisplay result={result} clearResults={clearResults} />}

      {batchResults && (
        <BatchResults batchResults={batchResults} clearResults={clearResults} />
      )}
    </>
  );
}
